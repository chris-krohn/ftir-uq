...
### Picrust data import into phyloseq
# "otu_table"
pws <- data.frame(read_tsv("./data/picrust_pathway_abundances_unstrat.tsv", col_names = TRUE)) %>% 
          rename(Id = "pathway") 
# "tax_table"
MC_names <- read_tsv("./data/picrust_pathway_names_metacy_full.tsv") %>% 
  dplyr::select(-ALTERNATIVE) %>%
  dplyr::select(-`SUPER-PATHWAYS`)%>% 
  dplyr::select(-NAMES) %>% 
  dplyr::rename(Pathway2 = "COMMON-NAME") 
# "tax_table new"
MC_names_new <- read_tsv("./data/picrust_pathway_names_metacyc_select.tsv") 
#Combine
MC_names <- MC_names_new %>% dplyr::full_join(MC_names, by = "Id") %>% 
  filter(Id %in% pws$Id) %>% 
  column_to_rownames("Id")

pws <- pws %>% column_to_rownames("Id")

sample_data(ps_1C)$SampleID <- rownames(data.frame(sample_data(ps_1C)))
metadata <- data.frame(sample_data(ps_1C))
...
## Create otu tables for Bushra - Microbes
```{r}
...
## Create otu tables for Bushra - Picrust2
```{r}
#check <- sample_data(ps.flt)
check <- data.frame(sample_data(ps_pw)) %>% rownames_to_column("ID") %>% dplyr::select(Date, AD, ID)
check.uq <- masterdata %>% dplyr::filter(UQ.ID != "") %>% dplyr::select(Date, SampleID.DNA, AD, UQ.ID)
check.both <- check.uq %>% dplyr::select(Date, SampleID.DNA, AD, UQ.ID) %>% left_join(check, by = c("Date", "AD")) %>% 
  dplyr::select(Date, SampleID.DNA, AD, UQ.ID, ID)
...
# PATHWAYS
ps.flt <- phyloseq::subset_samples(ps_pw, SampleID  %in% selection )
ps.flt <- prune_taxa(taxa_sums(ps.flt ) > 0, ps.flt )

otus <- data.frame(otu_table(ps.flt))
taxa <- data.frame(tax_table(ps.flt))
otu_table <- otus %>% rownames_to_column("ASV") %>% left_join(taxa %>% rownames_to_column("ASV"))
write.csv(otu_table, "./data/otutable_bushra_picrust_pathways_raw.csv", row.names = FALSE)
